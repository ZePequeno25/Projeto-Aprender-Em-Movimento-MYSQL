...
const getStudentsHandler = async (req, res) => {
  //Log do início da obtenção de dados dos alunos
  try {
    //Log do início da obtenção de dados dos alunos
...
const generateTeacherCode = async (req, res) => {
    //Log do início da geração de código de professor
    ...
    try{
        ...
        if(!await isProfessor(userId)){
            //Log do erro de usuário não é professor
            logger.warn(`❌ [relationshipController] Usuário ${userId} não é professor`, 'RELATIONSHIPS');
        ...
        //Log do código de professor gerado
...
const getTeacherCodeHandler = async (req, res) => {
    //Log do início da obtenção de código de professor
    ...
    try {
        ...
        if(!isValidId(teacherId, 'teacherId')){
            //Log do erro de ID do professor inválido
        ...
        //Log do código de professor encontrado
        ...
        //Log do erro ao carregar código
...
const linkStudentByCode = async (req, res) => {
    //Log do início da vinculação de aluno
    ...
    try {
        ...
        if(!await isStudent(userId)){
            //Log do erro de usuário não é aluno
        ...
        if(!codeData){
            //Log do erro de código inválido ou expirado
        ...
        if(existing.length > 0){
            //Log do erro de relação já existe
        ...
        //Log do aluno vinculado ao professor
...
const getTeacherStudentsHandler = async (req, res) => {
    //Log do início da obtenção de alunos vinculados ao professor
    try {
        //Obtém o ID do usuário atual
        const userId = await getCurrentUserId(req);
        //Log do ID do usuário atual
...
const getStudentRelationsHandler = async (req, res) => {
    //Log do início da obtenção de relações de aluno
    try{
        //Obtém o ID do aluno
        ...
        //Log do erro ao listar professores
...
const unlinkStudent = async (req, res) => {
    //Log do início da desvinculação
    ...
    try{
        ...
        if(!isValidId(relationId, 'relationId')){
            //Log do erro de ID da relação inválido
        ...
        if(relationRows.length === 0){
            //Log do erro de relação não encontrada
